...
| Embedding model | Zhipu `embedding-3` (2048 dimensions, fixed) |
...
- `document_chunks` — one row per chunk, with `vector(2048)` embedding column

## Key Constraints

- Embedding dimension is fixed at 2048 (Zhipu `embedding-3`). Switching models requires re-ingestion.
